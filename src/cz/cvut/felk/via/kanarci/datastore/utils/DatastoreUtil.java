...
public class DatastoreUtil implements IDatastoreUtil{
	...
	private <T> void makeObjectPersistent(T o){
		...
		try{
			pm.makePersistent(o);
			System.out.println("Object " + o.getClass().getSimpleName() + " saved in datastore");
		}finally{
			pm.close();
		}
	}
	...
	private void deleteAllPersistentObjects(Class<?> o){
		...
		try{
			...
			System.out.println("All " + o.getClass().getSimpleName() + " objects wiped out from datastore");
		}
	...
	private <T> void deletePersistentObject(T o, Key key){
		System.out.println("Deleting object " + o.getClass().getSimpleName() + " with key " + KeyFactory.keyToString(key));
		PersistenceManager pm = PMF.get().getPersistenceManager();
...
//		  }
	...
	private <T> List<T> getAllObjectsOrderedBy(Class<T> o, String orderValue){
		...
		while(st.hasMoreElements()){
			orderString = orderString.concat(st.nextElement().toString() + " asc, ");
		}
...
//			pm.getObjectById(o.getSimpleName(), key);
	...
	@Override
	public void addGoods(String name, Text description, double price, int numOfPieces, boolean visiblity, 
			Key supplier, List<Key> category){

		Goods goods = new Goods(name, description, price, numOfPieces, visiblity, supplier, category);
		makeObjectPersistent(goods);
	}
	
	@Override
	public void addGoods(String name, Text description, double price, int numOfPieces, 
	...
	public void addOrder(String state, Date estimatedDeliveryDate,
			...
			Contact deliveryContact, Contact billingContact, Key addedBy){
		
		Order order = new Order(state,estimatedDeliveryDate, deliveryDate, deliveryMethod, goodsInOrder, 
				deliveryContact, billingContact, addedBy);
		makeObjectPersistent(order);
	}
	
	@Override
	public void addOrder(Date creationDate, DeliveryMethod deliveryMethod, Date deliveryDate, 
			Date estimatedDeliveryDate,	List<Goods> goodsInOrder, Contact deliveryContact,
			Contact billingContact, Key modificatedBy, Key adddBy){
		
		Order order = new Order(creationDate, deliveryMethod, deliveryDate, estimatedDeliveryDate, 
				goodsInOrder, deliveryContact, billingContact, modificatedBy, adddBy);
		makeObjectPersistent(order);
	}
...
//		return getAllObjects(Category.class);
		return getAllObjectsOrderedBy(Category.class, "name parameterName parameterValue");
